# Example: bgp routemap rd rewrite

=== "Topology"

    ![Alt text](../d2/rout-bgp665/rout-bgp665.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp665](../clab/rout-bgp665/rout-bgp665.yml) file  
        3. Launch ContainerLab `rout-bgp665.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp665.yml  
        ```
        4. Destroy ContainerLab `rout-bgp665.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp665.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp665.tst` file [here](../tst/rout-bgp665.tst)  
        3. Launch `rout-bgp665.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp665 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp665.tst` test:  
