...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp106/rout-bgp106.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp106](../clab/rout-bgp106/rout-bgp106.yml) file  
        3. Launch ContainerLab `rout-bgp106.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp106.yml  
        ```
        4. Destroy ContainerLab `rout-bgp106.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp106.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp106.tst` file [here](../tst/rout-bgp106.tst)  
        3. Launch `rout-bgp106.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp106 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp106.tst` test:  
