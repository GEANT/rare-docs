...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp345/rout-bgp345.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp345](../clab/rout-bgp345/rout-bgp345.yml) file  
        3. Launch ContainerLab `rout-bgp345.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp345.yml  
        ```
        4. Destroy ContainerLab `rout-bgp345.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp345.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp345.tst` file [here](../tst/rout-bgp345.tst)  
        3. Launch `rout-bgp345.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp345 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp345.tst` test:  
