# Example: otherl3evpns ingress route filtering with routepolicy with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp846/rout-bgp846.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp846](../clab/rout-bgp846/rout-bgp846.yml) file  
        3. Launch ContainerLab `rout-bgp846.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp846.yml  
        ```
        4. Destroy ContainerLab `rout-bgp846.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp846.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp846.tst` file [here](../tst/rout-bgp846.tst)  
        3. Launch `rout-bgp846.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp846 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp846.tst` test:  
