...
=== "Topology"
    ...
    ![Alt text](../d2/mpls-polka18/mpls-polka18.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [mpls-polka18](../clab/mpls-polka18/mpls-polka18.yml) file  
        3. Launch ContainerLab `mpls-polka18.yml` topology:  

        ```
           containerlab deploy --topo mpls-polka18.yml  
        ```
        4. Destroy ContainerLab `mpls-polka18.yml` topology:  

        ```
           containerlab destroy --topo mpls-polka18.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `mpls-polka18.tst` file [here](../tst/mpls-polka18.tst)  
        3. Launch `mpls-polka18.tst` test:  

        ```
           java -jar ../../rtr.jar test tester mpls-polka18 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `mpls-polka18.tst` test:  
