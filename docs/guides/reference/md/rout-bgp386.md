...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp386/rout-bgp386.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp386](../clab/rout-bgp386/rout-bgp386.yml) file  
        3. Launch ContainerLab `rout-bgp386.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp386.yml  
        ```
        4. Destroy ContainerLab `rout-bgp386.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp386.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp386.tst` file [here](../tst/rout-bgp386.tst)  
        3. Launch `rout-bgp386.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp386 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp386.tst` test:  
