# Example: bgp interas ctp with multiple labels

=== "Topology"

    ![Alt text](../d2/rout-bgp692/rout-bgp692.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp692](../clab/rout-bgp692/rout-bgp692.yml) file  
        3. Launch ContainerLab `rout-bgp692.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp692.yml  
        ```
        4. Destroy ContainerLab `rout-bgp692.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp692.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp692.tst` file [here](../tst/rout-bgp692.tst)  
        3. Launch `rout-bgp692.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp692 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp692.tst` test:  
