...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp149/rout-bgp149.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp149](../clab/rout-bgp149/rout-bgp149.yml) file  
        3. Launch ContainerLab `rout-bgp149.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp149.yml  
        ```
        4. Destroy ContainerLab `rout-bgp149.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp149.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp149.tst` file [here](../tst/rout-bgp149.tst)  
        3. Launch `rout-bgp149.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp149 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp149.tst` test:  
