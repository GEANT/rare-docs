# Example: ctp colors over confed bgp

=== "Topology"

    ![Alt text](../d2/rout-bgp676/rout-bgp676.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp676](../clab/rout-bgp676/rout-bgp676.yml) file  
        3. Launch ContainerLab `rout-bgp676.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp676.yml  
        ```
        4. Destroy ContainerLab `rout-bgp676.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp676.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp676.tst` file [here](../tst/rout-bgp676.tst)  
        3. Launch `rout-bgp676.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp676 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp676.tst` test:  
