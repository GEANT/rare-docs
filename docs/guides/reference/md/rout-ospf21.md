...
=== "Topology"
    ...
    ![Alt text](../d2/rout-ospf21/rout-ospf21.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-ospf21](../clab/rout-ospf21/rout-ospf21.yml) file  
        3. Launch ContainerLab `rout-ospf21.yml` topology:  

        ```
           containerlab deploy --topo rout-ospf21.yml  
        ```
        4. Destroy ContainerLab `rout-ospf21.yml` topology:  

        ```
           containerlab destroy --topo rout-ospf21.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-ospf21.tst` file [here](../tst/rout-ospf21.tst)  
        3. Launch `rout-ospf21.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-ospf21 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-ospf21.tst` test:  
