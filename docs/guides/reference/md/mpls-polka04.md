...
=== "Topology"
    ...
    ![Alt text](../d2/mpls-polka04/mpls-polka04.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [mpls-polka04](../clab/mpls-polka04/mpls-polka04.yml) file  
        3. Launch ContainerLab `mpls-polka04.yml` topology:  

        ```
           containerlab deploy --topo mpls-polka04.yml  
        ```
        4. Destroy ContainerLab `mpls-polka04.yml` topology:  

        ```
           containerlab destroy --topo mpls-polka04.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `mpls-polka04.tst` file [here](../tst/mpls-polka04.tst)  
        3. Launch `mpls-polka04.tst` test:  

        ```
           java -jar ../../rtr.jar test tester mpls-polka04 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `mpls-polka04.tst` test:  
