...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp256/rout-bgp256.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp256](../clab/rout-bgp256/rout-bgp256.yml) file  
        3. Launch ContainerLab `rout-bgp256.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp256.yml  
        ```
        4. Destroy ContainerLab `rout-bgp256.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp256.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp256.tst` file [here](../tst/rout-bgp256.tst)  
        3. Launch `rout-bgp256.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp256 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp256.tst` test:  
