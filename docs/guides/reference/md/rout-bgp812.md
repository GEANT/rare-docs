# Example: l3otherevpns over bgp route server

=== "Topology"

    ![Alt text](../d2/rout-bgp812/rout-bgp812.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp812](../clab/rout-bgp812/rout-bgp812.yml) file  
        3. Launch ContainerLab `rout-bgp812.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp812.yml  
        ```
        4. Destroy ContainerLab `rout-bgp812.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp812.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp812.tst` file [here](../tst/rout-bgp812.tst)  
        3. Launch `rout-bgp812.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp812 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp812.tst` test:  
