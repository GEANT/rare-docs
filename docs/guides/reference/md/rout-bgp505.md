...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp505/rout-bgp505.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp505](../clab/rout-bgp505/rout-bgp505.yml) file  
        3. Launch ContainerLab `rout-bgp505.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp505.yml  
        ```
        4. Destroy ContainerLab `rout-bgp505.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp505.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp505.tst` file [here](../tst/rout-bgp505.tst)  
        3. Launch `rout-bgp505.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp505 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp505.tst` test:  
