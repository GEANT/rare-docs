...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp173/rout-bgp173.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp173](../clab/rout-bgp173/rout-bgp173.yml) file  
        3. Launch ContainerLab `rout-bgp173.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp173.yml  
        ```
        4. Destroy ContainerLab `rout-bgp173.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp173.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp173.tst` file [here](../tst/rout-bgp173.tst)  
        3. Launch `rout-bgp173.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp173 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp173.tst` test:  
