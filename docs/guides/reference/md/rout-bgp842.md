# Example: otherl3evpns ingress route filtering with routemap with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp842/rout-bgp842.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp842](../clab/rout-bgp842/rout-bgp842.yml) file  
        3. Launch ContainerLab `rout-bgp842.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp842.yml  
        ```
        4. Destroy ContainerLab `rout-bgp842.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp842.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp842.tst` file [here](../tst/rout-bgp842.tst)  
        3. Launch `rout-bgp842.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp842 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp842.tst` test:  
