...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp298/rout-bgp298.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp298](../clab/rout-bgp298/rout-bgp298.yml) file  
        3. Launch ContainerLab `rout-bgp298.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp298.yml  
        ```
        4. Destroy ContainerLab `rout-bgp298.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp298.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp298.tst` file [here](../tst/rout-bgp298.tst)  
        3. Launch `rout-bgp298.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp298 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp298.tst` test:  
