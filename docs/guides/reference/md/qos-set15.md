...
=== "Topology"
    ...
    ![Alt text](../d2/qos-set15/qos-set15.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [qos-set15](../clab/qos-set15/qos-set15.yml) file  
        3. Launch ContainerLab `qos-set15.yml` topology:  

        ```
           containerlab deploy --topo qos-set15.yml  
        ```
        4. Destroy ContainerLab `qos-set15.yml` topology:  

        ```
           containerlab destroy --topo qos-set15.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `qos-set15.tst` file [here](../tst/qos-set15.tst)  
        3. Launch `qos-set15.tst` test:  

        ```
           java -jar ../../rtr.jar test tester qos-set15 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `qos-set15.tst` test:  
