...
=== "Topology"
    ...
    ![Alt text](../d2/mpls-polka03/mpls-polka03.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [mpls-polka03](../clab/mpls-polka03/mpls-polka03.yml) file  
        3. Launch ContainerLab `mpls-polka03.yml` topology:  

        ```
           containerlab deploy --topo mpls-polka03.yml  
        ```
        4. Destroy ContainerLab `mpls-polka03.yml` topology:  

        ```
           containerlab destroy --topo mpls-polka03.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `mpls-polka03.tst` file [here](../tst/mpls-polka03.tst)  
        3. Launch `mpls-polka03.tst` test:  

        ```
           java -jar ../../rtr.jar test tester mpls-polka03 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `mpls-polka03.tst` test:  
