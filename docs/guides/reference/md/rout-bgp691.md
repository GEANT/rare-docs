# Example: bgp interas labels with multiple labels

=== "Topology"

    ![Alt text](../d2/rout-bgp691/rout-bgp691.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp691](../clab/rout-bgp691/rout-bgp691.yml) file  
        3. Launch ContainerLab `rout-bgp691.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp691.yml  
        ```
        4. Destroy ContainerLab `rout-bgp691.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp691.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp691.tst` file [here](../tst/rout-bgp691.tst)  
        3. Launch `rout-bgp691.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp691 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp691.tst` test:  
