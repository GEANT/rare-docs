# Example: l3evpns ingress route filtering with routepolicy with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp838/rout-bgp838.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp838](../clab/rout-bgp838/rout-bgp838.yml) file  
        3. Launch ContainerLab `rout-bgp838.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp838.yml  
        ```
        4. Destroy ContainerLab `rout-bgp838.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp838.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp838.tst` file [here](../tst/rout-bgp838.tst)  
        3. Launch `rout-bgp838.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp838 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp838.tst` test:  
