...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp463/rout-bgp463.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp463](../clab/rout-bgp463/rout-bgp463.yml) file  
        3. Launch ContainerLab `rout-bgp463.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp463.yml  
        ```
        4. Destroy ContainerLab `rout-bgp463.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp463.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp463.tst` file [here](../tst/rout-bgp463.tst)  
        3. Launch `rout-bgp463.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp463 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp463.tst` test:  
