...
=== "Topology"
    ...
    ![Alt text](../d2/crypt-sgt14/crypt-sgt14.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [crypt-sgt14](../clab/crypt-sgt14/crypt-sgt14.yml) file  
        3. Launch ContainerLab `crypt-sgt14.yml` topology:  

        ```
           containerlab deploy --topo crypt-sgt14.yml  
        ```
        4. Destroy ContainerLab `crypt-sgt14.yml` topology:  

        ```
           containerlab destroy --topo crypt-sgt14.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `crypt-sgt14.tst` file [here](../tst/crypt-sgt14.tst)  
        3. Launch `crypt-sgt14.tst` test:  

        ```
           java -jar ../../rtr.jar test tester crypt-sgt14 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `crypt-sgt14.tst` test:  
