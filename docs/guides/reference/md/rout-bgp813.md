# Example: l3otherevpns over bgp with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp813/rout-bgp813.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp813](../clab/rout-bgp813/rout-bgp813.yml) file  
        3. Launch ContainerLab `rout-bgp813.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp813.yml  
        ```
        4. Destroy ContainerLab `rout-bgp813.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp813.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp813.tst` file [here](../tst/rout-bgp813.tst)  
        3. Launch `rout-bgp813.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp813 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp813.tst` test:  
