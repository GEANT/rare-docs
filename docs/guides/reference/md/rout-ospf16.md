...
=== "Topology"
    ...
    ![Alt text](../d2/rout-ospf16/rout-ospf16.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-ospf16](../clab/rout-ospf16/rout-ospf16.yml) file  
        3. Launch ContainerLab `rout-ospf16.yml` topology:  

        ```
           containerlab deploy --topo rout-ospf16.yml  
        ```
        4. Destroy ContainerLab `rout-ospf16.yml` topology:  

        ```
           containerlab destroy --topo rout-ospf16.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-ospf16.tst` file [here](../tst/rout-ospf16.tst)  
        3. Launch `rout-ospf16.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-ospf16 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-ospf16.tst` test:  
