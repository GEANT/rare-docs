...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp092/rout-bgp092.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp092](../clab/rout-bgp092/rout-bgp092.yml) file  
        3. Launch ContainerLab `rout-bgp092.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp092.yml  
        ```
        4. Destroy ContainerLab `rout-bgp092.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp092.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp092.tst` file [here](../tst/rout-bgp092.tst)  
        3. Launch `rout-bgp092.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp092 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp092.tst` test:  
