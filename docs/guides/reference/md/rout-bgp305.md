...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp305/rout-bgp305.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp305](../clab/rout-bgp305/rout-bgp305.yml) file  
        3. Launch ContainerLab `rout-bgp305.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp305.yml  
        ```
        4. Destroy ContainerLab `rout-bgp305.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp305.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp305.tst` file [here](../tst/rout-bgp305.tst)  
        3. Launch `rout-bgp305.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp305 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp305.tst` test:  
