# Example: ibgp rr prefix withdraw with car

=== "Topology"

    ![Alt text](../d2/rout-bgp721/rout-bgp721.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp721](../clab/rout-bgp721/rout-bgp721.yml) file  
        3. Launch ContainerLab `rout-bgp721.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp721.yml  
        ```
        4. Destroy ContainerLab `rout-bgp721.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp721.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp721.tst` file [here](../tst/rout-bgp721.tst)  
        3. Launch `rout-bgp721.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp721 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp721.tst` test:  
