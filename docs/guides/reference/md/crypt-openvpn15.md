...
=== "Topology"
    ...
    ![Alt text](../d2/crypt-openvpn15/crypt-openvpn15.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [crypt-openvpn15](../clab/crypt-openvpn15/crypt-openvpn15.yml) file  
        3. Launch ContainerLab `crypt-openvpn15.yml` topology:  

        ```
           containerlab deploy --topo crypt-openvpn15.yml  
        ```
        4. Destroy ContainerLab `crypt-openvpn15.yml` topology:  

        ```
           containerlab destroy --topo crypt-openvpn15.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `crypt-openvpn15.tst` file [here](../tst/crypt-openvpn15.tst)  
        3. Launch `crypt-openvpn15.tst` test:  

        ```
           java -jar ../../rtr.jar test tester crypt-openvpn15 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `crypt-openvpn15.tst` test:  
