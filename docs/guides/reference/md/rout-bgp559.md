...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp559/rout-bgp559.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp559](../clab/rout-bgp559/rout-bgp559.yml) file  
        3. Launch ContainerLab `rout-bgp559.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp559.yml  
        ```
        4. Destroy ContainerLab `rout-bgp559.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp559.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp559.tst` file [here](../tst/rout-bgp559.tst)  
        3. Launch `rout-bgp559.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp559 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp559.tst` test:  
