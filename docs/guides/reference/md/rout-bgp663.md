# Example: octp ibgp fullmesh prefix withdraw

=== "Topology"

    ![Alt text](../d2/rout-bgp663/rout-bgp663.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp663](../clab/rout-bgp663/rout-bgp663.yml) file  
        3. Launch ContainerLab `rout-bgp663.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp663.yml  
        ```
        4. Destroy ContainerLab `rout-bgp663.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp663.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp663.tst` file [here](../tst/rout-bgp663.tst)  
        3. Launch `rout-bgp663.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp663 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp663.tst` test:  
