# Example: octp bgp ingress route filtering with routemap with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp653/rout-bgp653.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp653](../clab/rout-bgp653/rout-bgp653.yml) file  
        3. Launch ContainerLab `rout-bgp653.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp653.yml  
        ```
        4. Destroy ContainerLab `rout-bgp653.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp653.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp653.tst` file [here](../tst/rout-bgp653.tst)  
        3. Launch `rout-bgp653.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp653 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp653.tst` test:  
