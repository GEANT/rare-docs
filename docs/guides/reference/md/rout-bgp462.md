...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp462/rout-bgp462.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp462](../clab/rout-bgp462/rout-bgp462.yml) file  
        3. Launch ContainerLab `rout-bgp462.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp462.yml  
        ```
        4. Destroy ContainerLab `rout-bgp462.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp462.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp462.tst` file [here](../tst/rout-bgp462.tst)  
        3. Launch `rout-bgp462.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp462 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp462.tst` test:  
