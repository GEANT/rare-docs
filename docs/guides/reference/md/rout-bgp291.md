...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp291/rout-bgp291.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp291](../clab/rout-bgp291/rout-bgp291.yml) file  
        3. Launch ContainerLab `rout-bgp291.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp291.yml  
        ```
        4. Destroy ContainerLab `rout-bgp291.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp291.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp291.tst` file [here](../tst/rout-bgp291.tst)  
        3. Launch `rout-bgp291.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp291 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp291.tst` test:  
