# Example: bgp soft-reconfig with ctp

=== "Topology"

    ![Alt text](../d2/rout-bgp614/rout-bgp614.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp614](../clab/rout-bgp614/rout-bgp614.yml) file  
        3. Launch ContainerLab `rout-bgp614.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp614.yml  
        ```
        4. Destroy ContainerLab `rout-bgp614.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp614.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp614.tst` file [here](../tst/rout-bgp614.tst)  
        3. Launch `rout-bgp614.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp614 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp614.tst` test:  
