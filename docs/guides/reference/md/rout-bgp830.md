# Example: unicast+l3evpns over confed bgp with dynamic capability

=== "Topology"

    ![Alt text](../d2/rout-bgp830/rout-bgp830.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp830](../clab/rout-bgp830/rout-bgp830.yml) file  
        3. Launch ContainerLab `rout-bgp830.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp830.yml  
        ```
        4. Destroy ContainerLab `rout-bgp830.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp830.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp830.tst` file [here](../tst/rout-bgp830.tst)  
        3. Launch `rout-bgp830.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp830 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp830.tst` test:  
