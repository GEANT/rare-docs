# Example: ibgp rr prefix movement with ctp

=== "Topology"

    ![Alt text](../d2/rout-bgp630/rout-bgp630.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp630](../clab/rout-bgp630/rout-bgp630.yml) file  
        3. Launch ContainerLab `rout-bgp630.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp630.yml  
        ```
        4. Destroy ContainerLab `rout-bgp630.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp630.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp630.tst` file [here](../tst/rout-bgp630.tst)  
        3. Launch `rout-bgp630.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp630 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp630.tst` test:  
