# Example: bgp change in aggregator with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp797/rout-bgp797.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp797](../clab/rout-bgp797/rout-bgp797.yml) file  
        3. Launch ContainerLab `rout-bgp797.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp797.yml  
        ```
        4. Destroy ContainerLab `rout-bgp797.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp797.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp797.tst` file [here](../tst/rout-bgp797.tst)  
        3. Launch `rout-bgp797.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp797 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp797.tst` test:  
