...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp222/rout-bgp222.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp222](../clab/rout-bgp222/rout-bgp222.yml) file  
        3. Launch ContainerLab `rout-bgp222.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp222.yml  
        ```
        4. Destroy ContainerLab `rout-bgp222.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp222.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp222.tst` file [here](../tst/rout-bgp222.tst)  
        3. Launch `rout-bgp222.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp222 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp222.tst` test:  
