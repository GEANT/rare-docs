...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp133/rout-bgp133.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp133](../clab/rout-bgp133/rout-bgp133.yml) file  
        3. Launch ContainerLab `rout-bgp133.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp133.yml  
        ```
        4. Destroy ContainerLab `rout-bgp133.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp133.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp133.tst` file [here](../tst/rout-bgp133.tst)  
        3. Launch `rout-bgp133.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp133 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp133.tst` test:  
