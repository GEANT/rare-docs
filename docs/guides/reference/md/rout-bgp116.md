...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp116/rout-bgp116.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp116](../clab/rout-bgp116/rout-bgp116.yml) file  
        3. Launch ContainerLab `rout-bgp116.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp116.yml  
        ```
        4. Destroy ContainerLab `rout-bgp116.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp116.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp116.tst` file [here](../tst/rout-bgp116.tst)  
        3. Launch `rout-bgp116.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp116 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp116.tst` test:  
