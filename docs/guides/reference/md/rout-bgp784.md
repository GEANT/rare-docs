# Example: bgp change in validity

=== "Topology"

    ![Alt text](../d2/rout-bgp784/rout-bgp784.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp784](../clab/rout-bgp784/rout-bgp784.yml) file  
        3. Launch ContainerLab `rout-bgp784.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp784.yml  
        ```
        4. Destroy ContainerLab `rout-bgp784.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp784.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp784.tst` file [here](../tst/rout-bgp784.tst)  
        3. Launch `rout-bgp784.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp784 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp784.tst` test:  
