# Example: otherl3evpns egress route filtering with routemap with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp843/rout-bgp843.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp843](../clab/rout-bgp843/rout-bgp843.yml) file  
        3. Launch ContainerLab `rout-bgp843.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp843.yml  
        ```
        4. Destroy ContainerLab `rout-bgp843.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp843.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp843.tst` file [here](../tst/rout-bgp843.tst)  
        3. Launch `rout-bgp843.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp843 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp843.tst` test:  
