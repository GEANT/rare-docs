# Example: bgp additional path with php car

=== "Topology"

    ![Alt text](../d2/rout-bgp700/rout-bgp700.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp700](../clab/rout-bgp700/rout-bgp700.yml) file  
        3. Launch ContainerLab `rout-bgp700.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp700.yml  
        ```
        4. Destroy ContainerLab `rout-bgp700.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp700.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp700.tst` file [here](../tst/rout-bgp700.tst)  
        3. Launch `rout-bgp700.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp700 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp700.tst` test:  
