...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp213/rout-bgp213.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp213](../clab/rout-bgp213/rout-bgp213.yml) file  
        3. Launch ContainerLab `rout-bgp213.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp213.yml  
        ```
        4. Destroy ContainerLab `rout-bgp213.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp213.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp213.tst` file [here](../tst/rout-bgp213.tst)  
        3. Launch `rout-bgp213.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp213 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp213.tst` test:  
