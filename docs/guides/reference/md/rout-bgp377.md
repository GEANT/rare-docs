...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp377/rout-bgp377.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp377](../clab/rout-bgp377/rout-bgp377.yml) file  
        3. Launch ContainerLab `rout-bgp377.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp377.yml  
        ```
        4. Destroy ContainerLab `rout-bgp377.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp377.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp377.tst` file [here](../tst/rout-bgp377.tst)  
        3. Launch `rout-bgp377.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp377 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp377.tst` test:  
