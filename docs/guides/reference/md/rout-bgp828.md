# Example: unicast+l3evpns over ebgp with dynamic capability

=== "Topology"

    ![Alt text](../d2/rout-bgp828/rout-bgp828.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp828](../clab/rout-bgp828/rout-bgp828.yml) file  
        3. Launch ContainerLab `rout-bgp828.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp828.yml  
        ```
        4. Destroy ContainerLab `rout-bgp828.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp828.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp828.tst` file [here](../tst/rout-bgp828.tst)  
        3. Launch `rout-bgp828.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp828 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp828.tst` test:  
