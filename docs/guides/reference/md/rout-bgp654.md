# Example: octp bgp egress route filtering with routemap with soft-reconfig

=== "Topology"

    ![Alt text](../d2/rout-bgp654/rout-bgp654.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp654](../clab/rout-bgp654/rout-bgp654.yml) file  
        3. Launch ContainerLab `rout-bgp654.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp654.yml  
        ```
        4. Destroy ContainerLab `rout-bgp654.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp654.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp654.tst` file [here](../tst/rout-bgp654.tst)  
        3. Launch `rout-bgp654.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp654 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp654.tst` test:  
