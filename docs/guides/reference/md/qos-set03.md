...
=== "Topology"
    ...
    ![Alt text](../d2/qos-set03/qos-set03.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [qos-set03](../clab/qos-set03/qos-set03.yml) file  
        3. Launch ContainerLab `qos-set03.yml` topology:  

        ```
           containerlab deploy --topo qos-set03.yml  
        ```
        4. Destroy ContainerLab `qos-set03.yml` topology:  

        ```
           containerlab destroy --topo qos-set03.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `qos-set03.tst` file [here](../tst/qos-set03.tst)  
        3. Launch `qos-set03.tst` test:  

        ```
           java -jar ../../rtr.jar test tester qos-set03 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `qos-set03.tst` test:  
