...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp115/rout-bgp115.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp115](../clab/rout-bgp115/rout-bgp115.yml) file  
        3. Launch ContainerLab `rout-bgp115.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp115.yml  
        ```
        4. Destroy ContainerLab `rout-bgp115.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp115.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp115.tst` file [here](../tst/rout-bgp115.tst)  
        3. Launch `rout-bgp115.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp115 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp115.tst` test:  
