# Example: bgp route server with car

=== "Topology"

    ![Alt text](../d2/rout-bgp703/rout-bgp703.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp703](../clab/rout-bgp703/rout-bgp703.yml) file  
        3. Launch ContainerLab `rout-bgp703.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp703.yml  
        ```
        4. Destroy ContainerLab `rout-bgp703.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp703.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp703.tst` file [here](../tst/rout-bgp703.tst)  
        3. Launch `rout-bgp703.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp703 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp703.tst` test:  
