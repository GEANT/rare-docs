...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp315/rout-bgp315.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp315](../clab/rout-bgp315/rout-bgp315.yml) file  
        3. Launch ContainerLab `rout-bgp315.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp315.yml  
        ```
        4. Destroy ContainerLab `rout-bgp315.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp315.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp315.tst` file [here](../tst/rout-bgp315.tst)  
        3. Launch `rout-bgp315.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp315 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp315.tst` test:  
