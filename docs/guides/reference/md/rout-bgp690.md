# Example: bgp interas othervpn with multiple labels

=== "Topology"

    ![Alt text](../d2/rout-bgp690/rout-bgp690.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp690](../clab/rout-bgp690/rout-bgp690.yml) file  
        3. Launch ContainerLab `rout-bgp690.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp690.yml  
        ```
        4. Destroy ContainerLab `rout-bgp690.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp690.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp690.tst` file [here](../tst/rout-bgp690.tst)  
        3. Launch `rout-bgp690.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp690 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp690.tst` test:  
