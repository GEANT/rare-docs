...
=== "Topology"
    ...
    ![Alt text](../d2/conn-bridge15/conn-bridge15.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [conn-bridge15](../clab/conn-bridge15/conn-bridge15.yml) file  
        3. Launch ContainerLab `conn-bridge15.yml` topology:  

        ```
           containerlab deploy --topo conn-bridge15.yml  
        ```
        4. Destroy ContainerLab `conn-bridge15.yml` topology:  

        ```
           containerlab destroy --topo conn-bridge15.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `conn-bridge15.tst` file [here](../tst/conn-bridge15.tst)  
        3. Launch `conn-bridge15.tst` test:  

        ```
           java -jar ../../rtr.jar test tester conn-bridge15 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `conn-bridge15.tst` test:  
