# Example: bgp additional path with php ctp

=== "Topology"

    ![Alt text](../d2/rout-bgp610/rout-bgp610.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp610](../clab/rout-bgp610/rout-bgp610.yml) file  
        3. Launch ContainerLab `rout-bgp610.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp610.yml  
        ```
        4. Destroy ContainerLab `rout-bgp610.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp610.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp610.tst` file [here](../tst/rout-bgp610.tst)  
        3. Launch `rout-bgp610.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp610 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp610.tst` test:  
