# Example: vpns with egress rtfilter

=== "Topology"

    ![Alt text](../d2/rout-bgp591/rout-bgp591.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp591](../clab/rout-bgp591/rout-bgp591.yml) file  
        3. Launch ContainerLab `rout-bgp591.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp591.yml  
        ```
        4. Destroy ContainerLab `rout-bgp591.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp591.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp591.tst` file [here](../tst/rout-bgp591.tst)  
        3. Launch `rout-bgp591.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp591 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp591.tst` test:  
