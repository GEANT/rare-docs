...
=== "Topology"
    ...
    ![Alt text](../d2/mpls-polka09/mpls-polka09.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [mpls-polka09](../clab/mpls-polka09/mpls-polka09.yml) file  
        3. Launch ContainerLab `mpls-polka09.yml` topology:  

        ```
           containerlab deploy --topo mpls-polka09.yml  
        ```
        4. Destroy ContainerLab `mpls-polka09.yml` topology:  

        ```
           containerlab destroy --topo mpls-polka09.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `mpls-polka09.tst` file [here](../tst/mpls-polka09.tst)  
        3. Launch `mpls-polka09.tst` test:  

        ```
           java -jar ../../rtr.jar test tester mpls-polka09 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `mpls-polka09.tst` test:  
