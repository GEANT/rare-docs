# Example: car colors over bgp additional path

=== "Topology"

    ![Alt text](../d2/rout-bgp762/rout-bgp762.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp762](../clab/rout-bgp762/rout-bgp762.yml) file  
        3. Launch ContainerLab `rout-bgp762.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp762.yml  
        ```
        4. Destroy ContainerLab `rout-bgp762.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp762.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp762.tst` file [here](../tst/rout-bgp762.tst)  
        3. Launch `rout-bgp762.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp762 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp762.tst` test:  
