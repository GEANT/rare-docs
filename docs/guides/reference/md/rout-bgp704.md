# Example: bgp soft-reconfig with car

=== "Topology"

    ![Alt text](../d2/rout-bgp704/rout-bgp704.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp704](../clab/rout-bgp704/rout-bgp704.yml) file  
        3. Launch ContainerLab `rout-bgp704.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp704.yml  
        ```
        4. Destroy ContainerLab `rout-bgp704.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp704.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp704.tst` file [here](../tst/rout-bgp704.tst)  
        3. Launch `rout-bgp704.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp704 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp704.tst` test:  
