# Example: unicast+ocar over bgp with additional path

=== "Topology"

    ![Alt text](../d2/rout-bgp724/rout-bgp724.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp724](../clab/rout-bgp724/rout-bgp724.yml) file  
        3. Launch ContainerLab `rout-bgp724.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp724.yml  
        ```
        4. Destroy ContainerLab `rout-bgp724.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp724.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp724.tst` file [here](../tst/rout-bgp724.tst)  
        3. Launch `rout-bgp724.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp724 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp724.tst` test:  
