...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp097/rout-bgp097.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp097](../clab/rout-bgp097/rout-bgp097.yml) file  
        3. Launch ContainerLab `rout-bgp097.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp097.yml  
        ```
        4. Destroy ContainerLab `rout-bgp097.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp097.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp097.tst` file [here](../tst/rout-bgp097.tst)  
        3. Launch `rout-bgp097.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp097 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp097.tst` test:  
