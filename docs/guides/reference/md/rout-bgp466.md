...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp466/rout-bgp466.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp466](../clab/rout-bgp466/rout-bgp466.yml) file  
        3. Launch ContainerLab `rout-bgp466.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp466.yml  
        ```
        4. Destroy ContainerLab `rout-bgp466.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp466.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp466.tst` file [here](../tst/rout-bgp466.tst)  
        3. Launch `rout-bgp466.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp466 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp466.tst` test:  
