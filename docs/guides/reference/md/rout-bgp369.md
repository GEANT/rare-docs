...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp369/rout-bgp369.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp369](../clab/rout-bgp369/rout-bgp369.yml) file  
        3. Launch ContainerLab `rout-bgp369.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp369.yml  
        ```
        4. Destroy ContainerLab `rout-bgp369.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp369.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp369.tst` file [here](../tst/rout-bgp369.tst)  
        3. Launch `rout-bgp369.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp369 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp369.tst` test:  
