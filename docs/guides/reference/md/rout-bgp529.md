...
=== "Topology"
    ...
    ![Alt text](../d2/rout-bgp529/rout-bgp529.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp529](../clab/rout-bgp529/rout-bgp529.yml) file  
        3. Launch ContainerLab `rout-bgp529.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp529.yml  
        ```
        4. Destroy ContainerLab `rout-bgp529.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp529.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp529.tst` file [here](../tst/rout-bgp529.tst)  
        3. Launch `rout-bgp529.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp529 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp529.tst` test:  
