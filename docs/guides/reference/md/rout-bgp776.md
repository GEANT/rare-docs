# Example: unicast+vpns over ebgp with dynamic capability

=== "Topology"

    ![Alt text](../d2/rout-bgp776/rout-bgp776.svg)
...
=== "Emulation"
    ...
    === "ContainerLab"
        ...
        2. Fetch [rout-bgp776](../clab/rout-bgp776/rout-bgp776.yml) file  
        3. Launch ContainerLab `rout-bgp776.yml` topology:  

        ```
           containerlab deploy --topo rout-bgp776.yml  
        ```
        4. Destroy ContainerLab `rout-bgp776.yml` topology:  

        ```
           containerlab destroy --topo rout-bgp776.yml  
        ```
    ...
    === "freeRtr CLI"
        ...
        2. Fetch `rout-bgp776.tst` file [here](../tst/rout-bgp776.tst)  
        3. Launch `rout-bgp776.tst` test:  

        ```
           java -jar ../../rtr.jar test tester rout-bgp776 path ./ temp ./ wait
        ```
        4. Destroy freeRtr `rout-bgp776.tst` test:  
